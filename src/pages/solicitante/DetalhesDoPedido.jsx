...
// Adicionado Polyline e useMap para controle da rota
...
export default function DetalhesDoPedido() {
  ...
  const [rotaCaminho, setRotaCaminho] = useState([]); // Armazena a linha da rota
  ...
  useEffect(() => {
    async function carregarPedido() {
      try {
        ...
        // Calcular rota se houver coordenadas de origem e destino
            ...
            <div className="bg-white p-6 rounded-xl shadow border border-gray-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">
                  {pedido.titulo} <span className="text-blue-600 font-mono text-lg">#{pedido.id}</span>
                </h1>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button onClick={handleCancelarPedido} className="px-4 py-2 bg-red-600 text-white font-semibold rounded-lg text-sm hover:bg-red-700 transition flex items-center shadow-md shadow-red-100">
              ...
              <div className="lg:col-span-1 space-y-6">
                <div className="bg-white p-6 rounded-xl shadow border border-gray-300">
                  ...
                  <div className="space-y-3">
                    <DetalheItem label="Peso Estimado" value={`${pedido.peso_kg} kg`} icon="weight-hanging" />
                    <DetalheItem label="Entregador Alocado" value={pedido.entregador?.nome || "Aguardando propostas"} icon="user-circle" />
                    <DetalheItem label="Valor do Serviço" value={`Kz ${pedido.valor_final ?? pedido.valor_sugerido}`} icon="dollar-sign" />
                  </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-white p-4 rounded-xl shadow border border-gray-300">
                  <h3 className="text-lg font-bold text-blue-700 mb-4 flex items-center">
                    <i className="fas fa-map-marked-alt mr-3"></i> Localização e Rota
                  </h3>

                  <div className="w-full h-80 rounded-lg overflow-hidden border border-gray-200 z-10 relative">
                    <MapContainer
                      center={[pedido.origem_latitude || -8.8399, pedido.origem_longitude || 13.2894]}
                      zoom={13}
                    ...
                    >
                      ...
                      {pedido.origem_latitude && (
                        <Marker position={[pedido.origem_latitude, pedido.origem_longitude]} icon={customIcon}>
                          <Popup><b>Origem:</b><br/>{pedido.origem_endereco}</Popup>
                        </Marker>
                      )}

                      {/* Marcador de Destino */}
                      {pedido.destino_latitude && (
                        <Marker position={[pedido.destino_latitude, pedido.destino_longitude]} icon={customIcon}>
                          <Popup><b>Destino:</b><br/>{pedido.destino_endereco}</Popup>
                        </Marker>
                      )}
                      ...
                      {/* Ajuste automático do foco */}
                      <FitRoute positions={rotaCaminho.length > 0 ? rotaCaminho : [[pedido.origem_latitude, pedido.origem_longitude], [pedido.destino_latitude, pedido.destino_longitude]]} />
                    </MapContainer>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white p-5 rounded-xl shadow border-l-4 border-l-green-500 border-gray-300">
                    <h4 className="font-bold text-sm text-green-700 flex items-center mb-2 uppercase tracking-wider">
                      <i className="fas fa-location-arrow mr-2"></i> Ponto de Recolha
                    </h4>
                    <p className="text-sm text-gray-700 font-medium">{pedido.origem_endereco}</p>
                  </div>
                  ...
                  <div className="bg-white p-5 rounded-xl shadow border-l-4 border-l-red-500 border-gray-300">
                    <h4 className="font-bold text-sm text-red-700 flex items-center mb-2 uppercase tracking-wider">
                      <i className="fas fa-flag-checkered mr-2"></i> Ponto de Entrega
                    </h4>
                    <p className="text-sm text-gray-700 font-medium">{pedido.destino_endereco}</p>
                  </div>
                </div>
              </div>
            </div>
