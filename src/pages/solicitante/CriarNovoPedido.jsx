...
export default function CriarNovoPedido() {
  ...
  const buscarEnderecos = async (query, setSugestoes) => {
    if (query?.length > 3) {
      try {
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${query}&countrycodes=ao&limit=5`);
  ...
  useEffect(() => {
    const timer = setTimeout(() => buscarEnderecos(watchOrigem, setSugestoesOrigem), 600);
    return () => clearTimeout(timer);
  }, [watchOrigem]);

  useEffect(() => {
    const timer = setTimeout(() => buscarEnderecos(watchDestino, setSugestoesDestino), 600);
    return () => clearTimeout(timer);
  }, [watchDestino]);
            ...
            <div className="bg-white border border-gray-300 rounded-xl shadow-lg overflow-hidden">
              <div className="bg-blue-600 p-6">
                <h2 className="text-xl font-bold text-white flex items-center">
                  <i className="fas fa-plus-circle mr-3"></i> Solicitar Nova Entrega
                </h2>
                <p className="text-blue-100 text-sm mt-1">Preencha os dados abaixo para encontrar um entregador disponível.</p>
              </div>

              <form onSubmit={handleSubmit(onSubmit)} className="p-6 sm:p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1 md:col-span-2">
                    <label className="text-sm font-semibold text-gray-700">Título do Pedido *</label>
                    <input type="text" {...register("titulo")} placeholder="Ex: Entrega de Documentos Contábeis" className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all ${errors.titulo ? "border-red-500 bg-red-50" : "border-gray-300"}`} />
                    {errors.titulo && <p className="text-red-500 text-xs mt-1">{errors.titulo.message}</p>}
                  </div>

                  <div className="space-y-1 md:col-span-2">
                    <label className="text-sm font-semibold text-gray-700">Descrição Detalhada *</label>
                    <textarea rows="3" {...register("descricao")} placeholder="Descreva o que será entregue..." className={`w-full p-3 border rounded-lg resize-none focus:ring-2 focus:ring-blue-500 outline-none transition-all ${errors.descricao ? "border-red-500 bg-red-50" : "border-gray-300"}`} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1">
                    ...
                    <input type="number" step="0.01" {...register("peso_kg")} placeholder="0.00" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-sm font-semibold text-gray-700">Urgência</label>
                    <select {...register("urgencia")} className="w-full p-3 border border-gray-300 rounded-lg bg-white focus:ring-2 focus:ring-blue-500 outline-none">
                  ...
                  <div className="space-y-1">
                    <label className="text-sm font-semibold text-gray-700">Valor Sugerido (AKZ)</label>
                    <input type="number" {...register("valor_sugerido")} placeholder="Ex: 2500" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                  </div>
                </div>

                <div className="relative py-4">
                  <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-gray-200"></span></div>
                  <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-gray-400 font-semibold">Logística e Rota</span></div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1 relative">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <button type="button" onClick={handleGetLocation} className="text-xs text-blue-600 hover:text-blue-800 font-semibold flex items-center"><i className={`fas ${localizando ? "fa-spinner fa-spin" : "fa-crosshairs"} mr-1`}></i> Usar minha localização</button>
                      </div>
                      <textarea rows="2" {...register("origem")} placeholder="Endereço de retirada..." className={`w-full resize-none p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all ${errors.origem ? "border-red-500 bg-red-50" : "border-gray-300"}`} />
                      ...
                      {sugestoesOrigem.length > 0 && (
                        <div className="absolute z-9999 w-full bg-white border border-gray-300 rounded-b-lg shadow-lg max-h-48 overflow-y-auto">
                          {sugestoesOrigem.map((s, i) => (
                            <div key={i} onClick={() => selecionarEndereco(s, "origem")} className="p-2 hover:bg-gray-100 cursor-pointer text-sm border-b last:border-0">{s.display_name}</div>
                          ))}
                        </div>
                      )}
                    </div>
                      ...
                      {sugestoesDestino.length > 0 && (
                        <div className="absolute z-9999 w-full bg-white border border-gray-300 rounded-b-lg shadow-lg max-h-48 overflow-y-auto">
                          {sugestoesDestino.map((s, i) => (
                            <div key={i} onClick={() => selecionarEndereco(s, "destino")} className="p-2 hover:bg-gray-100 cursor-pointer text-sm border-b last:border-0">{s.display_name}</div>
                          ))}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="w-full h-[350px] rounded-xl border border-gray-300 overflow-hidden relative z-0">
                    <MapContainer center={mapCenter} zoom={13} scrollWheelZoom={false} style={{ height: "100%", width: "100%" }}>
                      ...
                      <Marker position={mapCenter}><Popup>Origem</Popup></Marker>
                      {destinoCoords.lat && <Marker position={[destinoCoords.lat, destinoCoords.lng]}><Popup>Destino</Popup></Marker>}
                      
                      {/* TRAÇADO DA ROTA (Linha Azul) */}
                      {rotaCaminho.length > 0 && (
                        <Polyline positions={rotaCaminho} color="#3b82f6" weight={5} opacity={0.7} />
                      )}
                    </MapContainer>
                  </div>
                </div>
                ...
                <div className="pt-6 flex flex-col sm:flex-row gap-4 justify-end">
                  <button type="button" onClick={() => navigate(-1)} className="px-6 py-3 cursor-pointer bg-gray-100 text-gray-600 font-bold rounded-lg hover:bg-gray-200 transition-colors">Cancelar</button>
                  <button type="submit" disabled={loading} className={`px-10 py-3 cursor-pointer bg-blue-600 text-white font-bold rounded-lg shadow-lg flex items-center justify-center transition-all ${loading ? "opacity-70 cursor-not-allowed" : "hover:bg-blue-700 active:scale-95"}`}>
                    {loading ? <><i className="fas fa-spinner fa-spin mr-2"></i> Processando...</> : "Confirmar Pedido"}
                  </button>
                </div>
              </form>
            </div>
