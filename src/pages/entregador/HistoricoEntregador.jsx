...
export default function HistoricoEntregador() {
    ...
    async function carregarHistorico() {
      try {
        ...
        const formatados = dados.map((p) => ({
          id: p.id,
          dataExibicao: new Date(p.criado_em).toLocaleDateString("pt-BR"),
          ...
          valorNumerico:
            p.status === "ENTREGUE" ? parseFloat(p.valor_sugerido) : 0,
          origem: p.origem_endereco,
          destino: p.destino_endereco,
          dataObjeto: new Date(p.criado_em),
        }));
  ...
  const corridasFiltradas = useMemo(() => {
    ...
    return corridasHistorico.filter((corrida) => {
      if (filtroPeriodo === "Total") return true;

      if (filtroPeriodo === "Mês") {
        return isSameMonth(corrida.dataObjeto, hoje);
      }

      if (filtroPeriodo === "Semana") {
        const seteDiasAtras = startOfDay(subDays(hoje, 7));
        return isWithinInterval(corrida.dataObjeto, {
          start: seteDiasAtras,
          end: endOfDay(hoje),
        });
      }
  ...
  }, [corridasHistorico, filtroPeriodo]);

  // Cálculos do Resumo baseados no filtro atual
  const resumoGanhos = useMemo(() => {
    const total = corridasFiltradas
      .filter((c) => c.status === "Concluído")
      .reduce((acc, curr) => acc + curr.valorNumerico, 0);

    return {
      total: `AOA ${total.toLocaleString("pt-BR")}`,
      corridas: corridasFiltradas.length,
    };
  }, [corridasFiltradas]);

  const registrosParaExibir = isExpanded
    ? corridasFiltradas
    : corridasFiltradas.slice(0, 6);
      ...
      <div className="flex-1 flex flex-col md:ml-64 h-screen relative overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                ...
                {["Semana", "Mês", "Total"].map((periodo) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setIsExpanded(false); // Reseta a expansão ao trocar filtro
                    }}
          ...
          <section className="space-y-4 pb-10">
            ...
            {loading ? (
              ...
              <>
                {registrosParaExibir.map((corrida) => (
                  <div
                    key={corrida.id}
                    className="bg-white p-4 sm:p-5 rounded-xl shadow-md flex flex-col sm:flex-row justify-between items-start sm:items-center border-l-4 gap-4 transition-all"
                    style={{
                      borderColor:
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center text-sm font-semibold flex-wrap gap-2">
                        <span className="text-gray-500">
                          {corrida.dataExibicao} | {corrida.hora}
                        </span>
                        <span
                    ...
                    <div className="text-left sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0 w-full sm:w-auto">
                      <p className="text-xl font-bold text-gray-900">
                        AOA {corrida.valorNumerico.toLocaleString("pt-BR")}
                      </p>
                ...
                {corridasFiltradas.length > 6 && (
                  <div className="pt-4">
                    ...
                    >
                      ...
                      {isExpanded
                        ? "VER MENOS"
                        : `VER MAIS (${corridasFiltradas.length - 6})`}
                    </button>
                  </div>
                )}
              </>
            ...
            {!loading && corridasFiltradas.length === 0 && (
            ...
            {!loading && corridasFiltradas.length > 0 && (
              <div className="flex justify-end pt-4 border-t border-gray-300">
                <p className="text-gray-600 text-sm">
                  ...
                  {corridasFiltradas.length} no período {filtroPeriodo}.
                </p>
              </div>
            )}
          </section>
